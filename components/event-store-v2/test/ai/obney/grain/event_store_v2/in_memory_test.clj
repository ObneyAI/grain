(ns ai.obney.grain.event-store-v2.in-memory-test
...
(deftest cas-predicate-true-stores-events
  (let [event (es/->event {:type :test/alpha :tags #{} :body {:n 1}})
    ...
    ;; CAS success in in-memory does NOT create a :grain/tx event
...
(deftest tx-metadata-nil-when-not-provided
  (append-event! :test/alpha #{} {:n 1})
  (let [tx (first (tx-events (read-events {})))]
    ;; In-memory: :metadata key is present but nil (differs from Postgres where key is absent)
    (is (nil? (:metadata tx)))))
