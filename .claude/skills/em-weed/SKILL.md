...
| `:flow/illegal-connection` | error | model bug | Flow step violates the CQRS grammar. Rewrite the step to legal adjacency (command→event, event→read-model/todo-processor, read-model→query/command/screen, query→screen, screen→command, todo-processor→command, periodic-task→command/event). |
...
- **A green `:produces`/`:reads` is agreement, not proof.** Those edges are confirmed only against the def-site *annotation*, never the handler body. The declaration↔behaviour gap is closed by executable Given/When/Then (`em-propagate`), not by `weed`. Only `event→read-model` and `event→todo-processor` are confirmable against actual live wiring; every other flow edge is checked for endpoint existence + grammar only.
